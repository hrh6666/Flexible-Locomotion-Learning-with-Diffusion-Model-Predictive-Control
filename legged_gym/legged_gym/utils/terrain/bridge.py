...
class TerrainBridge:
    def __init__(self, cfg, num_envs):
        ...
        self.max_bridge_width=cfg.max_bridge_width
        self.min_bridge_width=cfg.min_bridge_width
    ...
    def generate_fractal_noise_2d(xSize=20, ySize=20, xSamples=1600, ySamples=1600, \
        ...
        for _ in range(fractalOctaves):
            noise += amplitude * TerrainBridge.generate_perlin_noise_2d((xSamples, ySamples), (xScale, yScale)) * zScale
    ...
    def curiculum(self):
        for j in range(self.cfg.num_cols):
            for i in range(self.cfg.num_rows):
                ...
                # if True:
                if j<=5:
                    bridge_width=(self.max_bridge_width-i/(self.cfg.num_rows-1)*(self.max_bridge_width-self.min_bridge_width))
                    platform_size=np.random.uniform(self.min_platform_size,self.max_platform_size)
                    terrain = self.make_terrain_bridge(bridge_width,platform_size,zScale=0.05)
                else:
                    ...
                    terrain = self.make_terrain_perlin(zScale)
                
                self.add_terrain_to_map(terrain, i, j) 

    def make_terrain_bridge(self,bridge_width,platform_size,zScale=0.05):
        terrain = terrain_utils.SubTerrain("terrain",
                                width=self.width_per_env_pixels,
                                length=self.length_per_env_pixels,
                                vertical_scale=self.vertical_scale,
                                horizontal_scale=self.horizontal_scale)
        self.bridge_terrain(terrain, bridge_width=bridge_width, platform_size=platform_size,zScale=zScale)
        return terrain
    ...
    def bridge_terrain(self, terrain, bridge_width, platform_size, zScale=0.05):
        # switch parameters to discrete units
        bridge_width = round(bridge_width / terrain.horizontal_scale)
        bridge_height = round(-5 / terrain.vertical_scale)
        platform_size = round(platform_size / terrain.horizontal_scale)
        border_width = round(1 / terrain.horizontal_scale)
        
        # start_out=round((terrain.width-platform_size-gap_width)/2)
        start_in=round((terrain.width-platform_size)/2)
        # end_out=round((terrain.width+platform_size+gap_width)/2)
        end_in=round((terrain.width+platform_size)/2)
        
        start_br=round((terrain.width-bridge_width)/2)
        end_br=round((terrain.width+bridge_width)/2)
        
        terrain.height_field_raw[:, :] = bridge_height
        
        terrain.height_field_raw[start_in: end_in, start_in: end_in] = 0.
        terrain.height_field_raw[end_in:, start_br: end_br] = 0.
