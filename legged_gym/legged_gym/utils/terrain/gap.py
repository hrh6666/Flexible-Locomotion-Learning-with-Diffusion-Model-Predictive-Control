...
class TerrainGap:
    def __init__(self, cfg, num_envs):
        ...
        self.max_gap_dis=cfg.max_gap_dis
        self.min_gap_dis=cfg.min_gap_dis
    ...
    def generate_fractal_noise_2d(xSize=20, ySize=20, xSamples=1600, ySamples=1600, \
        ...
        for _ in range(fractalOctaves):
            noise += amplitude * TerrainGap.generate_perlin_noise_2d((xSamples, ySamples), (xScale, yScale)) * zScale
    ...
    def curiculum(self):
        for j in range(self.cfg.num_cols):
            for i in range(self.cfg.num_rows):
                ...
                # if True:
                if j<=5:
                    gap_dis=(self.min_gap_dis+i/(self.cfg.num_rows-1)*(self.max_gap_dis-self.min_gap_dis))
                    platform_size=np.random.uniform(self.min_platform_size,self.max_platform_size)
                    terrain = self.make_terrain_gap(gap_dis,platform_size,zScale=0.05)
                else:
    ...
    def make_terrain_gap(self,gap_dis,platform_size,zScale=0.05):
        terrain = terrain_utils.SubTerrain("terrain",
                                width=self.width_per_env_pixels,
                                length=self.length_per_env_pixels,
                                vertical_scale=self.vertical_scale,
                                horizontal_scale=self.horizontal_scale)
        self.gap_terrain(terrain, gap_dis=gap_dis, platform_size=platform_size,zScale=zScale)
        return terrain
    ...
    def gap_terrain(self, terrain, gap_dis, platform_size, zScale=0.05):
        # switch parameters to discrete units
        gap_width = round(gap_dis / terrain.horizontal_scale)
        gap_height = round(-2 / terrain.vertical_scale)
        ...
        terrain.height_field_raw[start_out: end_out, start_out: end_out] = gap_height
