...
class TerrainMultitaskTrack:
    ...
    def generate_fractal_noise_2d(xSize=20, ySize=20, xSamples=1600, ySamples=1600, \
        ...
        for _ in range(fractalOctaves):
            noise += amplitude * TerrainMultitaskTrack.generate_perlin_noise_2d((xSamples, ySamples), (xScale, yScale)) * zScale
    ...
    def stumble_terrain_border(self, terrain, num_pit, num_pole, zScale=0.05):
        ...
        if not self.cfg.single_terrain:
            track_length = self.env_length/8 / (num_pit+2)
        else:
        ...
        for k in range(num_pit):
            ...
            if not self.cfg.single_terrain:
                center_y = round((3*self.env_length/8+ (k+1)*track_length)/terrain.horizontal_scale)
            else:
    ...
    def add_obstacle_to_sim(self, gym, sim, device="cpu"):
        if not self.cfg.single_terrain:
            track_length_bar = self.env_length/8 / (self.num_bar_track+2)
        else:
            track_length_bar = self.env_length/(self.num_bar_track+2)
            
        if not self.cfg.single_terrain:
            track_length_baffle = self.env_length/8 / (self.num_baffle_track+2)
        else:
            track_length_baffle = self.env_length/(self.num_baffle_track+2)
        for j in range(self.cfg.num_cols):
            for i in range(self.cfg.num_rows):
                for k in range(self.num_bar_track):
                    ...
                    center_y = -self.env_length/2 + (k+2)*track_length_bar
                ...
                baffle_start_height = 0.32
                baffle_end_height = 0.22
                baffle_height_diff = baffle_start_height - baffle_end_height
                width = 0.3
                
                # Iterate over each baffle track, using linear interpolation to set the baffle's height.
                for k in range(self.num_baffle_track):
                    if self.num_baffle_track > 1:
                        height_ = baffle_start_height - (k / (self.num_baffle_track - 1)) * baffle_height_diff + width / 2
                    else:
                        height_ = baffle_start_height

                    # Define a fixed baffle thickness.
                    baffle_thickness = 0.1
                    # Randomly generate the baffle length within a specified range.
                    length = self.env_width
                    # Define the size of the horizontal baffle:
                    # - Length along the x-axis.
                    # - Thickness along the y and z axes.
                    size = np.array([length, baffle_thickness, width], dtype=np.float32)
                    # Compute the y-coordinate for the baffle based on its track position.
                    center_y = -self.env_length / 2 + self.env_length / 8 + (k + 2) * track_length_baffle
                    # Center the baffle along the x-axis (x = length/2) and set its height.
                    center = np.array([length / 2, center_y, height_], dtype=np.float32) + self.env_origins[i, j]
                    center = np.array(center, dtype=np.float32)

                    # Create the trimesh for the baffle obstacle and add it to the simulation.
                    obstacle_trimesh = trimesh.box_trimesh(size, center)
                    self.add_trimesh_to_sim(gym, sim, obstacle_trimesh)
