...
class TerrainObstacle:
    ...
    def generate_fractal_noise_2d(xSize=20, ySize=20, xSamples=1600, ySamples=1600, \
        ...
        for _ in range(fractalOctaves):
            noise += amplitude * TerrainObstacle.generate_perlin_noise_2d((xSamples, ySamples), (xScale, yScale)) * zScale
        ...
        for j in range(self.cfg.num_cols):
            for i in range(self.cfg.num_rows):
                ...
                if (j%2)==0:
                    ...
                    obstacle_height=-(self.min_obstacle_height+i/(self.cfg.num_rows-1)*(self.max_obstacle_height-self.min_obstacle_height))
                    terrain=terrain_utils.discrete_obstacles_terrain(terrain, max_height=obstacle_height, min_size=2, max_size=4, num_rects=8, platform_size=1.)
