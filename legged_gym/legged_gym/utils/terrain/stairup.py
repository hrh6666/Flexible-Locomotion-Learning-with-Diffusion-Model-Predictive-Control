...
class TerrainStairUp:
    ...
    def generate_fractal_noise_2d(xSize=20, ySize=20, xSamples=1600, ySamples=1600, \
        ...
        for _ in range(fractalOctaves):
            noise += amplitude * TerrainStairUp.generate_perlin_noise_2d((xSamples, ySamples), (xScale, yScale)) * zScale
    ...
    def curiculum(self):
        for j in range(self.cfg.num_cols):
            for i in range(self.cfg.num_rows):
                if j<self.cfg.num_cols//2-1:
                # if True:
                    step_width=(self.max_step_width-j/(self.cfg.num_cols-1)*(self.max_step_width-self.min_step_width))
                    step_height=-(self.min_step_height+i/(self.cfg.num_rows-1)*(self.max_step_height-self.min_step_height))
                ...
                elif j>=self.cfg.num_cols//2-1 and j<self.cfg.num_cols-2:
                    step_width=(self.max_step_width-j/(self.cfg.num_cols-1)*(self.max_step_width-self.min_step_width))
                    step_height=-(self.min_step_height+i/(self.cfg.num_rows-1)*(0.1-self.min_step_height))
                    platform_size=np.random.uniform(self.min_platform_size,self.max_platform_size)
                    border_width=self.border_width
                    terrain = self.make_terrain_stair(step_width,step_height,platform_size,border_width,zScale=0.05)
                else:
