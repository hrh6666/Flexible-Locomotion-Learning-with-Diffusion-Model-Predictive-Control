...
class TerrainStumble:
    def __init__(self, cfg, num_envs):
        ...
        self.square_obstacle = cfg.square_obstacle
        self.square_length = cfg.square_length
        self.min_square_bar_height = cfg.min_square_bar_height
        self.max_square_bar_height = cfg.max_square_bar_height
        self.min_square_obstacle_width = cfg.min_square_obstacle_width
        self.max_square_obstacle_width = cfg.max_square_obstacle_width
    ...
    def generate_fractal_noise_2d(xSize=20, ySize=20, xSamples=1600, ySamples=1600, \
        ...
        for _ in range(fractalOctaves):
            noise += amplitude * TerrainStumble.generate_perlin_noise_2d((xSamples, ySamples), (xScale, yScale)) * zScale
            ...
            for j in range(self.cfg.num_cols):
                for i in range(self.cfg.num_rows):
                    ...
                    if True:
                        ...
                        zScale=(self.min_zScale+i/(self.cfg.num_rows-1)*(self.max_zScale-self.min_zScale))
    ...
    def stumble_terrain_border(self, terrain, num_pit, num_pole, length, width, platform_size, border_width, zScale=0.05):
        ...
        if not self.square_obstacle:
            for _ in range(num_pit):
                ...
                height_ = round(np.random.uniform(0.1, 1.0)/terrain.vertical_scale)
                if np.random.rand() > 0.5:
                    length_, width_ = width_, length_

                start_i = round(np.random.uniform(border_width_, i-width_-border_width_))
                start_j = round(np.random.uniform(border_width_, j-length_-border_width_))
                terrain.height_field_raw[start_i:start_i+width_, start_j:start_j+length_] = -height_
        else:   
            width_ = round(np.random.uniform(width*0.5, width*1.5) / terrain.horizontal_scale)
            length_ = round(np.random.uniform(length*0.5, length*1.5) / terrain.horizontal_scale)
            height_ = round(np.random.uniform(0.1, 1.0)/terrain.vertical_scale)
        ...
        for _ in range(num_pole):
            width_ = round(np.random.uniform(width*0.3, width*1.5)/terrain.horizontal_scale)
    ...
    def add_obstacle_to_sim(self, gym, sim, device="cpu"):
        if not self.square_obstacle:
            for j in range(self.cfg.num_cols):
                for i in range(self.cfg.num_rows):
                    # if j<self.cfg.num_cols//2:
                    if True:
                        num_bar=round(self.min_num_bar+i/(self.cfg.num_rows-1)*(self.max_num_bar-self.min_num_bar))
                        length=self.pit_length
                        width=self.pit_width
                        platform_size=np.random.uniform(0.5*self.platform_size, 1.5*self.platform_size)
                        border_width=self.border_width
                        for _ in range(num_bar):
                            width_ = np.random.uniform(width*0.1, width*1)
                            length_ = np.random.uniform(length*0.5, length*1.5)
                            height_ = np.random.uniform(0.1, 0.2)
                            width_save = width_
                            if np.random.rand() > 0.5:
                                length_, width_ = width_, length_
        ...
        for j in range(self.cfg.num_cols):
            for i in range(self.cfg.num_rows):
                # if j<self.cfg.num_cols//2:
                if True:
                    num_bar=round(self.min_num_bar+i/(self.cfg.num_cols-1)*(self.max_num_bar-self.min_num_bar))
                    length=self.pit_length
                    width=self.pit_width
                    platform_size=np.random.uniform(0.5*self.platform_size, 1.5*self.platform_size)
                    border_width=self.border_width
                    for _ in range(num_bar):
                        width_ = np.random.uniform(width*0.1, width*1)
                        length_ = np.random.uniform(length*0.5, length*1.5)
                        height_ = 0.+i/(self.cfg.num_rows-1)*(0.15-0.)
                        width_save = width_
                        if np.random.rand() > 0.5:
                            length_, width_ = width_, length_

                            start_i = np.random.uniform(self.border_width, self.env_width-width_-border_width)
                            # while start_i>self.env_width/2-0.5*platform_size-width_ and start_i<self.env_width/2+0.5*platform_size-width_:
                            #     start_i = np.random.uniform(self.border_width, self.env_width-width_-border_width)

                            start_j = np.random.uniform(self.border_width, self.env_length-length_-border_width)
                            # while start_j>self.env_length/2-0.5*platform_size-length_ and start_j<self.env_length/2+0.5*platform_size-length_:
                            #     start_j = np.random.uniform(self.border_width, self.env_length-length_-border_width)
                            
                            # Debug
                            # width_=width_save=0.05
                            # length_=2
                            # start_i=6
                            # start_j=4

                            size=np.array([width_, length_, width_save], dtype= np.float32)
                            center=np.array([start_i-self.env_width/2+width_/2, start_j-self.env_length/2+length_/2, height_], dtype= np.float32)+self.env_origins[i, j]
