...
class TerrainStumbleOld:
    ...
    def generate_fractal_noise_2d(xSize=20, ySize=20, xSamples=1600, ySamples=1600, \
        ...
        for _ in range(fractalOctaves):
            noise += amplitude * TerrainStumbleOld.generate_perlin_noise_2d((xSamples, ySamples), (xScale, yScale)) * zScale
            ...
            for j in range(self.cfg.num_cols):
                for i in range(self.cfg.num_rows):
                    ...
                    if True:
                        ...
                        platform_size=np.random.uniform(0.5*self.platform_size, 1.2*self.platform_size)
                ...
                for i in range(self.cfg.num_rows):
                    if self.cfg.unify:
                        ...
                        # if True:
                            ...
                            width=0.05 + i/(self.cfg.num_rows-1)*(0.25 -0.05)
    ...
    def stumble_terrain_border(self, terrain, num_pit, num_pole, length, width, platform_size, border_width, zScale=0.05):
        ...
        if not self.square_obstacle:
            for _ in range(num_pit):
                width_ = round(np.random.uniform(width*1, width*5.5)/terrain.horizontal_scale)
                length_ = round(np.random.uniform(length*0.8, length*1.8)/terrain.horizontal_scale)
                ...
                for i in range(self.cfg.num_rows):
                    ...
                    if True:
                        ...
                        for _ in range(num_bar):
                            width_ = np.random.uniform(width*0.05, width*0.8)
                            length_ = np.random.uniform(length*0.8, length*1.8)
                            height_ = 0.+i/(self.cfg.num_rows-1)*(0.25-0.)
                            width_save = width_
                            if np.random.rand() > 0.5:
                                length_, width_ = width_, length_
