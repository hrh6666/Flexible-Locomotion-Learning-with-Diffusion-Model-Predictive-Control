...
class TerrainStumbleSquare:
    ...
    def generate_fractal_noise_2d(xSize=20, ySize=20, xSamples=1600, ySamples=1600, \
        ...
        for _ in range(fractalOctaves):
            noise += amplitude * TerrainStumbleSquare.generate_perlin_noise_2d((xSamples, ySamples), (xScale, yScale)) * zScale
            ...
            for i in range(self.cfg.num_rows):
                ...
                if True:
                    ...
                    if j<self.cfg.stair_cols:
                        zScale=(self.min_zScale+j/(self.cfg.stair_cols-1)*(self.max_zScale-self.min_zScale))
                    else:
            ...
            for j in range(self.cfg.num_cols):
                for i in range(self.cfg.num_rows):
                    ...
                    # if True:
                        ...
                        # size=np.array([width_, length_, width_save], dtype= np.float32)
                        # center=np.array([0, 0, height_], dtype= np.float32)+self.env_origins[i, j]
                        # center=np.array(center, dtype=np.float32)
